...
#### 修复可选链操作符使用不当 (src/main.js:810)
- 修复前：`window.__TAURI__?.core || {}`
- 修复后：`(window.__TAURI__?.core) || {}`
- 影响：避免逻辑错误，提高代码正确性

#### 修复滚动同步除零风险 (src/main.js:1971, 1981)
- 在 `syncScroll` 和 `syncScrollReverse` 函数中添加除零检查
- 当内容不足以滚动时提前返回，避免异常
- 影响：提高稳定性，防止运行时错误

#### 修复正则表达式后行断言兼容性问题 (src/main.js:1121)
- 修复前：使用后行断言 `(?<!...)`（部分浏览器不支持）
- 修复后：使用捕获组方式 `(^|[^\w"'\/\-])((https?:\/\/)...)`
- 影响：提高浏览器兼容性，支持更多浏览器

#### 修复文件读取无大小限制问题 (src-tauri/src/commands.rs:36)
- 添加文件大小检查，限制最大10MB
- 防止打开超大文件导致内存溢出
- 影响：提高安全性，防止内存溢出攻击
